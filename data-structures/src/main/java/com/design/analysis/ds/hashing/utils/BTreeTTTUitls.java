...
public class BTreeTTTUitls<K2 extends Comparable<K2>, K3, V> {

	// BTreeLTUitls<K> btltUtils = new BTreeLTUitls<>();

	@SuppressWarnings({ "unchecked", "rawtypes" })
	public HK3TTTNode<K2, K3, V> createBst(HK3TTTNode<K2, K3, V> node, K2 key2) {

		if (node == null)
			node = new HK3TTTNode(key2);
		else {
			// replacing
	...
	// Key2 tree pring
	public void inOrderPrint(HK3TTTNode<K2, K3, V> node) {
		if (node != null) {
			inOrderPrint(node.lt);
			System.out.print("\nkey2:-(" + node.k + ")->");
			inInOrderPrint(node.inY);
			inOrderPrint(node.rt);
		}
	}

	// Key3 tree print
	public void inInOrderPrint(HK2TTNode<K3, V> node) {
		if (node != null) {
	...
	public HK3TTTNode<K2, K3, V> recvSearch(HK3TTTNode<K2, K3, V> node, K2 key2) {
		if (node != null) {
	...
	public HK3TTTNode<K2, K3, V> delete(HK3TTTNode<K2, K3, V> node, K2 key) {
		if (node == null) {
