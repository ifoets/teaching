...
public interface IArrangeReArrange {
     ...
     * if i is odd and j < i
     **/
    public void evenPosIncrOddPosDecr(int []a, int n);

    /** it realize it require wave form sorting start from low */
    public void evenPosIncrOddPosDecrX(int []a, int n);

    public void evenPosIncrOddPosIncr(int []a, int n);
    ...
    //after separation of -/+v swap 2-2 position
    ...
    // @See 5.
    /** 23. Replace every array element by multiplication of previous and next **/
    public void modify(int []a, int n);

    /** 24. Shuffle a given array **/
    public void shuffleArr(int []a, int n);
     ...
     * sorted arrays,
     */
    List<List<Integer>> generateAllArrays(int []a, int []b);
    /*
     * 40. Minimum number of swaps required for arranging pairs adjacent to each
     * other
     */
    int minNoOfSwapFindPartner(int []a, Map<Integer,Integer> partner);
