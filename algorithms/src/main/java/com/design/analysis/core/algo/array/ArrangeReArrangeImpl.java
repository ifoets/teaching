...
public class ArrangeReArrangeImpl implements IArrangeReArrange {
    ...
    //keep even as it is and reverse the odd pos find both side for i an j
    @Override
    ...
    @Override
    public void alternativePosNevElemX(int []a){
    ...
    // IDEA arrange like separate +ve/-ve together just a[i]-k>0 at the place of
    // a[i]>0
    ...
    public String formBiggestNo(int []a) {
        ...
        List<BigestInt> numList = new ArrayList<>();
    ...
    public void threeWayPartition(int []a, int lowVal, int hiVal) {
         ...
         */
        @Override
        public List<List<Integer>> generateAllArrays(int []a, int []b) {
            ...
            for (int i = 2; i < a.length + b.length;i++) {
                List<List<Integer>> lList = new ArrayList<>(rList);
                for (List<Integer> l : lList) {
                    if (l.size() == i) {
                        int x = AlgoUtils.searchGreaterInSorted(flag ? a : b, l.get(l.size() - 1));
                        if (x != -1) {
         ...
         * other
         */
        public int minNoOfSwapFindPartner(int []a, Map<Integer,Integer> partner){

            Map<Integer,Integer> newMap = new HashMap<>(partner);
            for (Map.Entry<Integer,Integer> entry:partner.entrySet())
            {
            ...
            {
                if(!(a[i+1]==newMap.get(a[i]) || a[i]==newMap.get(a[i+1]))) {
                    int partnerIndex = AlgoUtils.searchIndex(a, newMap.get(a[i]));
                    int temp = a[i + 1];
                    a[i + 1] = a[partnerIndex];
                    a[partnerIndex] = temp;
                    minSwapCount++;
                }
            }
    ...
    /** 1. sort an array to create a temple*/
    @Override
    public void sortCreateTemple(int[] a){
        AlgoUtils.sortByN(a);
        AlgoUtils.reversePart(a,a.length/2,a.length-1);
    }
    /** 2. sort an array to create a cone */
    @Override
    public void sortCreateCone(int[] a){
        AlgoUtils.sortByN(a);
        AlgoUtils.reversePart(a,0, a.length/2);
    }

    /**3 sort like a wave of stock market going up*/
    public void sortStockWaveUp(int []a){
        AlgoUtils.sortByN(a);
        AlgoUtils.swapAlternative(a);

    }
    /** sort like a wave stock market going down */
    public void sortStockWaveDown(int []a){
        AlgoUtils.sortByN(a);
        AlgoUtils.reversePart(a,0,a.length-1);
        AlgoUtils.swapAlternative(a);
    }
}
